...
pipeline {
  ...
  parameters {
    ...
    string(name: 'EC2_HOST', defaultValue: '', description: 'Blank → credential obs-ec2-host')
    string(name: 'CONTAINER_PORT', defaultValue: '8082', description: 'product-service port')
    string(name: 'HEALTH_ENDPOINT', defaultValue: '/products', description: 'Health path on EC2 loopback')
    string(name: 'EC2_USER', defaultValue: 'ec2-user', description: 'SSH user')
    ...
    booleanParam(name: 'REQUIRE_2XX_HEALTH', defaultValue: true, description: 'Health requires 2xx/3xx')
  }
